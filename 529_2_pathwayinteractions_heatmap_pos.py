# Create heatmap of pathway responses with fraction of responding genes with log2-fcs above a cutoff of log2 fc 1.0 upon knockdown of genes
...
# Add L-aspartate biosynthesis II back in as a column and a row of zeros, as it is the only pathway that doesnt exist here but it does in the negative response heatmap
...
print(f"Number of free CPU cores: {num_cores}")
...
heatmap_data_excel_1.to_excel("04_pathway_interactions_cutoff_1.0.xlsx", sheet_name=f"fraction_responses_over_{cutoff}", index=False)
heatmap_data = pd.read_excel("04_pathway_interactions_cutoff_1.0.xlsx", f"fraction_responses_over_{cutoff}", index_col="pathway")
heatmap_data["L-aspartate biosynthesis"]=0
heatmap_data["NAD phosphorylation and dephosphorylation"]=0
...
#common_kept_pathways.sort()
common_kept_pathways_01.append("L-aspartate biosynthesis")
common_kept_pathways_01.append("NAD phosphorylation and dephosphorylation")
...
heatmap_data_clipped = heatmap_data_matched_filtered.clip(upper=0.3)
...
with pd.ExcelWriter(output_excel_path) as writer: 
    ...
    heatmap_data_excel_4.to_excel(writer, sheet_name=f"pathway_interactions_clip_0.3", index=False)
...
if not heatmap_data_clipped.empty:
    ...
    sns.heatmap(heatmap_data_clipped, cmap='PiYG_r', center=0, annot=False,
                linewidths=.1, linecolor='gray', cbar_kws={'ticks': [0, 0.1, 0.2, 0.3], 'label': 'Pathway response: fraction'}, square=True, ax=ax_heatmap, cbar_ax=cbar_ax)
    ...
    plt.savefig(os.path.join(output_dir, f"04_pathway_interactions_heatmap_{cutoff}_and_{co2}_{mini}_03.png"))
...
pathways_responses_above_zero_per_kd = (heatmap_data_matched_filtered_2 > 0).sum(axis=1)
pathways_responses_above_zero_per_resp = (heatmap_data_matched_filtered_2 > 0).sum(axis=0)
respnum_df_dict = {'Num >0 as kd': pathways_responses_above_zero_per_kd,
                'Num >0 as resp': pathways_responses_above_zero_per_resp}
num_resp_df = pd.DataFrame(respnum_df_dict)
num_resp_df["pathway"]=num_resp_df.index
print(num_resp_df)
df_legend=pd.merge(df_legend, num_resp_df, on='pathway', how="left")

df_legend.to_excel("04_heatmap_legend.xlsx", "heatmap legend")
print("Made Number Legend for heatmap")

num_high_self_resp=0
kd_pathways=[col for col in heatmap_data_clipped.columns if not col=="pathway"]
for kd_pathway in kd_pathways:
    resp_own_kd=heatmap_data_clipped.loc[heatmap_data_clipped.index==kd_pathway, kd_pathway]
    if resp_own_kd.values>=0.1:
        num_high_self_resp+=1
print(f"There are {num_high_self_resp} high self-responses")
...
if not heatmap_data_clipped.empty:
    ...
    heatmap_plot = sns.heatmap(heatmap_data_clipped, cmap=mapcolor, center=0, annot=False,
                             linewidths=width, linecolor='black', cbar_kws={'ticks': [0, 0.1, 0.2, 0.3]}, square=True, ax=ax_heatmap, cbar_ax=cbar_ax)
    ...
    plt.tight_layout()
    plt.rcParams['svg.fonttype'] = 'none'
    plt.savefig(os.path.join(output_dir, f"04_pathway_interactions_heatmap_{cutoff}_and_{co2}_{mini}_num_{mapcolor}_{width}.png"))
    plt.savefig(os.path.join(output_dir, f"04_pathway_interactions_heatmap_{cutoff}_and_{co2}_{mini}_num_{mapcolor}_{width}.svg"))
    plt.close(fig)
else:
    print(f"No relevant data after filtering found. No heatmap made.")
